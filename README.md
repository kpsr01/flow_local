...
The local LFM model turns dictated coding requests into direct, structured prompts without inventing requirements. Common explicit request prefixes such as “could you please fix” become “fix”; questions ending in a question mark, uncertainty, and scope limits are retained. Coding cleanup uses llama.cpp constrained decoding to retain substantive words, order, case, identifiers, constraints, and dictated plan steps. Mixed imperative requests get separate constraint, step, and output clauses when recognizable; quoted content and conditional requests keep flexible formatting to avoid imposing new scope. Short requests stay concise. The model cannot generate an answer, code, a solution, or additional plan steps. Validation remains in place; a failed or truncated completion immediately falls back to the raw transcript instead of repeating the same deterministic inference. History cleanup retries use the saved coding target, not whichever harness happens to be active later.

Provider-qualified IDs from Pi and Oh My Pi, such as `openai-codex/gpt-5.3-codex` and `anthropic/claude-opus-5`, resolve to the corresponding bundled guide while preserving the captured model identity. Unrecognized providers and models retain the generic policy.
