...
- The cleanup model runs through LLamaSharp's CPU backend by default (`GpuLayerCount = 0`) with an 8192-token context, temperature-0 decoding with a repeat penalty, and no runtime tuning UI. Setting `FLOWLOCAL_LFM_GPU=1` opts into full GPU offload, which was observed to be slower and process-fatal on an MX450-class GPU; it remains available for experimentation and falls back to CPU automatically.
