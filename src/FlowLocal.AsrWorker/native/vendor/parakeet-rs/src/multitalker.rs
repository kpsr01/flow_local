...
impl MultitalkerASR {
    ...
    pub fn from_pretrained<P: AsRef<Path>, Q: AsRef<Path>>(
        ...
        sortformer.chunk_len = 14;
        sortformer.right_context = 0;
        ...
        Ok(Self {
            ...
            audio_processed: 0,
            chunk_idx: 0,
            prior_activity: Array2::zeros((PRE_ENCODE_CACHE, NUM_SPEAKERS)),
        })
    }
    ...
    /// Decode only tokens from an authenticated span of a speaker channel.
    pub fn transcript_since(&self, speaker_id: usize, seconds: f32) -> String {
        let first_frame = (seconds / SECONDS_PER_ENCODED_FRAME).floor() as usize;
        self.speakers.iter().find(|speaker| speaker.speaker_id == speaker_id)
            .map(|speaker| self.vocab.decode(&speaker.accumulated_tokens.iter()
                .filter(|&&(token, frame)| token < VOCAB_SIZE && frame >= first_frame)
                .map(|&(token, _)| token).collect::<Vec<_>>()))
            .unwrap_or_default()
    }
    ...
    pub fn transcribe_chunk(&mut self, audio_chunk: &[f32]) -> Result<Vec<SpeakerTranscript>> {
        self.transcribe_chunk_with_activity(audio_chunk, |_, _| {})
    }

    /// Activity and audio belong to the same contiguous encoder window.
    pub fn transcribe_chunk_with_activity(
        &mut self,
        audio_chunk: &[f32],
        mut on_activity: impl FnMut(&[f32], &Array2<f32>),
    ) -> Result<Vec<SpeakerTranscript>> {
        if audio_chunk.len() != self.chunk_audio_samples() {
            return Err(Error::Audio("Streaming requires complete, contiguous encoder windows".into()));
        }
        ...
        if raw_preds.predictions.nrows() * 8 != chunk_size {
            return Err(Error::Model(format!("Sortformer produced {} 80ms frames for {} mel frames", raw_preds.predictions.nrows(), chunk_size)));
        }
        let diar_preds = Array2::from_shape_fn((chunk_size, NUM_SPEAKERS), |(frame, speaker)| {
            raw_preds.predictions[[frame / 8, speaker]]
        });
        on_activity(audio_chunk, &diar_preds);
        ...
        for spk_id in 0..self.config.max_speakers {
            ...
            if max_activity > self.config.activity_threshold
                || self.speakers.iter().any(|s| s.speaker_id == spk_id)
            {
        ...
        self.prior_activity.assign(&diar_preds.slice(s![chunk_size - PRE_ENCODE_CACHE.., ..]));
    ...
    fn derive_speaker_targets(
        ...
        // Activity for the nine pre-encoder cache frames belongs to the prior window.

        let mut spk_vals = Vec::with_capacity(encoder_time);
        let mut bg_vals = Vec::with_capacity(encoder_time);

        for enc_t in 0..encoder_time {
            let (activity, diar_t) = if enc_t < PRE_ENCODE_CACHE {
                (&self.prior_activity, enc_t)
            } else {
                (diar_preds, enc_t - PRE_ENCODE_CACHE)
            };

            if diar_t < activity.nrows() && speaker_id < activity.ncols() {
                let spk_val = activity[[diar_t, speaker_id]];
                let bg_val = (0..activity.ncols())
                    .filter(|&j| j != speaker_id)
                    .map(|j| activity[[diar_t, j]])
                    .fold(0.0f32, f32::max);
