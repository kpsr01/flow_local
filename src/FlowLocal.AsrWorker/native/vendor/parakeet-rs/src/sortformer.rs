...
impl Sortformer {
    ...
    pub fn diarize_chunk_raw(
        ...
        let features = self.extract_mel_features(audio_16k_mono)?;
        // Centered STFT emits one extra frame; it belongs to the next audio window.
        let frames = (audio_16k_mono.len() / HOP_LENGTH).min(features.shape()[1]);
        let full_preds = self.process_features(&features.slice(s![.., ..frames, ..]).to_owned())?;
